	...
	if (isset($_POST['delete'])) {
		if (isset($_POST['checkbox'])) {
			...
			foreach ($checkbox as $id) {
				$sql = "DELETE FROM khachsan_phanhoi WHERE MaPhanHoi_KS = '$id'";
				$conn->query($sql);
				header("location: reply.php?MaKhachSan={$makhachsan}");
			}
		}
	}
?>
						...
						if(isset($_SESSION['username'])) {
							...
							echo '
								 ...
								 <li class="nav-item">
									 ...
									 <span class="nav-info" style="color: #8889AA;">Quản lý tour</span>
									 </a>
								 </li>
								 <li class="nav-item">
									 <a href="hotel.php" class="nav-link">
									 <i class="nav-icon fas fa-hotel"></i>
									 <span class="nav-info" style="color: #8889AA;">Quản lý khách sạn</span>
									 </a>
								 </li>
								 <li class="nav-item">
									 <a href="" class="nav-link">
									 <i class="nav-icon fas fa-utensils"></i>
									 <span class="nav-info"style="color: #8889AA;" >Quản lý nhà hàng</span>
									 </a>
								 </li>
								 <li class="nav-item">
									 <a href="" class="nav-link">
									 <i class="nav-icon fas fa-motorcycle"></i>
									 <span class="nav-info"style="color: #8889AA;" >Quản lý phương tiện</span>
									 </a>
								 </li>
							 	</ul>
							 <hr>
								...
								<div class="main-content-section">
									<div class="container">
										<div class="grid wide">
											...
											<div class="col-12">
											<?php 
												$sql_user = "SELECT DISTINCT khachhang.MaKhachHang, khachhang.TenKhachHang,khachhang.SDT FROM `khachsan_phanhoi`,`khachhang` where makhachsan='$makhachsan' AND khachsan_phanhoi.MaKhachHang=khachhang.MaKhachHang";
												$sql_phanhoi ="SELECT DISTINCT thoigian from khachsan_phanhoi where makhachsan=$makhachsan";
											?>
											<?php
											if ($result_reply->num_rows > 0) {
												?>
											<div style="margin: auto;width: 50%;border: 3px solid #ff891e;padding: 10px;">
												<h2>Xóa Phản Hồi Của Người Dùng</h2>
												<form action="delete_reply.php" method="GET">
													<select name="delete_reply" class="delete_reply" id="delete_reply">
														<?php  
															$result_user = $conn->query($sql_user);
															if ($result_user->num_rows  > 0) {
																while ($row_user = $result_user->fetch_assoc()) {
																	echo "<option value='{$row_user['MaKhachHang']}'>{$row_user['TenKhachHang']}-0{$row_user['SDT']}</option>";
																}
															}
														?>
													</select>
													<input type="submit" name="delete_user_reply" class="delete_reply" style="width: 200px;" value="Xóa Phản Hồi của User" >
												</form>
											</div>
											<h2>Xóa theo danh sách</h2>
												<form action="" method="post">
												<input type="submit" name="delete" value="Xóa" class="delete_reply" style="width:150px">
												...
												while ($row_reply = $result_reply->fetch_assoc()) {//Doc du lieu tung dong
													echo "
													<tr>
														<td><input type=checkbox name='checkbox[]' value='{$row_reply['MaPhanHoi_KS']}'>
